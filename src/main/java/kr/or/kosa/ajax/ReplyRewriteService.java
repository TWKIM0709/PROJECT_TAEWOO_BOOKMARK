	...
	private void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			if(request.getParameter("type").equals("blog")) {
				...
				int blog_reply_no = Integer.parseInt(request.getParameter("blog_reply_no"));
				...
				blog_reply.setBlog_no(blog_no);
				blog_reply.setBlog_reply_no(blog_reply_no);
				blog_reply.setId(id);
				blog_reply.setReply_content(content);
				
				int row = dao.replyRewrite(blog_reply);
				//작성 성공시
				if(row>0) {
